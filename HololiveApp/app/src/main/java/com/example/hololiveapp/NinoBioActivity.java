...
/**
 * Class that contains the layout and operations of the activity_nino_bio.xml
 */
public class NinoBioActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linLay = findViewById(R.id.LinearNino);
        getApiData();
        createImageViews();
    }
    ...
    public void getApiData(){
        Thread apiThr = new Thread(() -> {
            try {
                URL url = new URL("http://10.0.2.2:8080/holotopia_backend_war/hololive-members/6714/videos");
        ...
        Thread imgThr = new Thread(() -> {
            ...
            runOnUiThread(() -> {
                for (int x = 0; x < videolist.size(); x++) {
                    ImageView img = new ImageView(NinoBioActivity.this);
                    ...
                    img.setOnClickListener(v -> {
                        Intent intent = new Intent(NinoBioActivity.this, MediaActivity.class);
                        intent.putExtra("id",youtubeId);
                        startActivity(intent);
                    });
                    linLay.addView(img);
                    Glide.with(NinoBioActivity.this)
